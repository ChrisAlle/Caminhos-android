                                         ...
                                         new()  // indica que a classe específica terá um construtor vazio
{
    ...
    //Método para ler o vetor de cidades e criar a árvore de busca
    public void LerVetor(Dado[] cidades, int tamanho)
    {
        //Cria o objeto dado e chama o método particionar a partir do meio do vetor
        Dado dado = new Dado();
        Particionar(0, tamanho - 1, ref raiz);
        ...
        void Particionar(long inicio, long fim, ref NoArvore<Dado> atual)
        {
            if (inicio <= fim)
            {
                ...
                dado = cidades[meio];       // cria um objeto para armazenar os dados
                atual = new NoArvore<Dado>(dado);

                //Guarda a posição da cidade no vetor original (o mesmo vetor armazenado em Grafo)
                atual.PosicaoNoVetor = (int)meio;
